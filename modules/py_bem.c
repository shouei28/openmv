...
void BEM(int width, int height, int **events, int event_size,
         int8_t *output_buffer) {
  // Step 1: Clear output buffer
  memset(output_buffer, 0, width * height);

  // Step 2: Accumulate ON/OFF events directly
...
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(py_bem_obj, 5, 5,
                                           py_bem); // 5 args now
